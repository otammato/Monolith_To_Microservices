...
In this project we will deploy a monolithic node.js application to a Docker container, then decouple the application into microservices deployed in the AWS ECS (Elastic Container Services) cluster. The node.js application hosts a simple message board with threads and messages between users.
